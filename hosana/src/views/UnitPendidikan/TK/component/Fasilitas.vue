...
const fetchFasilitas = async () => {
  try {
    const res = await axios.get(`${API_BASE_URL}/api/v1/fasilitas/all?unit=TK`);
...
<template>
<div class="mx-auto px-5 md:px-15 pb-10 bg-(--gstk)">    
  <div class="cont-text1 text-center mb-5" data-aos="fade-down">
      <h2 class="font-bold  text-(--tk) pt-5 md:text-4xl text-3xl">FASILITAS</h2>
    </div>
    ...
    <div class="cont-swiper text-justify relative" data-aos="zoom-in">
      ...
      >
        <SwiperSlide
          ...
          :key="item.id"
        > 
          <div class="bg-[#f7caf4] hover:bg-white shadow-lg rounded-xl p-5 w-[100%] duration-800 hover:z-10">
            ...
            <h3 class="text-l font-bold mt-4 pb-2 text-(--tk)">
                {{ item.namaFasilitas }}
              </h3>
