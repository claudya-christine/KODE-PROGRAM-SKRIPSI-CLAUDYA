...
<template>
  <header class="sticky top-0 z-50 bg-(--smp) py-4 px-5 md:px-0">
    <div class="container mx-auto flex justify-between items-center md:px-10">
      ...
      <nav 
        ...
        bg-(--smp) md:bg-transparent md:flex md:items-center 
      ...
      >
        <ul class="text-white text-xs md:flex md:items-center py-3 md:py-0 px-1">
          <li 
            v-for="link in Links"
            :key="link.name"
            class="relative group hover:border-b-2 hover:border-(--primary) hover:text-(--primary)"
          >
            <div 
              v-if="link.children"
              ...
              :class="{
                'font-bold text-xs text-(--primary)': link.children.some(child => route.path === child.link)
              }"
            >
              {{ link.name }}
            </div>
            ...
            <ul 
              ...
              class="bg-[#d1931d] md:bg-white md:absolute md:top-full md:left-0 md:mt-2 shadow-lg md:min-w-[200px] z-50 transition duration-300 ease-in-out"
            >
