...
const fetchKegiatan = async () => {
  try {
    const res = await axios.get(`${API_BASE_URL}/api/v1/kegiatan/all?unit=SD`);
...
<template>
  <div class="mx-auto px-5 pb-15">
    ...
    <h1  class ="font-bold pt-10 md:pt-0 md:text-4xl text-2xl text-(--sd) ">
      KEGIATAN KAMI
    </h1>
    <h3 class="text-[#04301e] text-sm">Kegiatan seru dan ekstrakurikuler menarik untuk mengembangkan potensi, bakat, dan jiwa kepemimpinan siswa SD.
</h3>
</div>
    <div
    ...
    >
      ...
      >
        <!-- Judul -->
        <div
          class="w-full h-[40px] shadow-lg  rounded-lg bg-(--gssd) flex items-center justify-center"
        >
          <h3 class="text-xl font-bold text-(--sd)">
            {{ item.namaKegiatan }}
          </h3>
        </div>
        ...
        <!-- Konten -->
        <div
          class="w-full mt-3 p-5 rounded-lg h-full shadow-lg  bg-(--gssd)"
        >
      <div class="w-full mb-3 rounded-lg overflow-hidden group">
        ...
        /> -->
          ...
          <!-- Deskripsi -->
          <div class="rounded-lg bg-(--gssd) h-full text-xs text-justify">
            <p>{{ item.deskripsi }}</p>
          </div>
        </div>
      </div>
