...
const fetchAlumniSD = async () => {
  try {
    ...
    const sdData = allData.filter(
      (item) => item.unit?.pendidikan?.toLowerCase().includes("sd")
    );

    news.value = sdData.map((item) => ({
      id: item.id,
      image: WA,
      title: `SDS PROTESTAN Generasi ${item.generasi}`,
    ...
    <div class="cont-swiper text-justify relative" data-aos="zoom-in">
      ...
      >
        <SwiperSlide
          ...
          :key="item.id"
        >
          <div class="bg-white hover:bg-gray-50 shadow-lg rounded-xl p-5 w-[100%] duration-800 hover:z-10">
