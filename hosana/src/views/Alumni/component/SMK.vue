...
const fetchAlumniSMK = async () => {
  try {
    ...
    const smkData = allData.filter(
      (item) => item.unit?.pendidikan?.toLowerCase().includes("smk")
    );

    news.value = smkData.map((item) => ({
      id: item.id,
      image: WA,
      title: `SMKS HOSANA Generasi ${item.generasi}`,
...
<template>
  <div class="mx-auto px-5 md:px-15 pb-5">
    ...
    <div class="cont-swiper text-justify relative" data-aos="zoom-in">
      ...
      <!-- Tombol sama persis dengan alumni SD -->
