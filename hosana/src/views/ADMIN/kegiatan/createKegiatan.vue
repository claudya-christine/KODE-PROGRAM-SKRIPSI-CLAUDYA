...
const addKegiatan = async () => {
  if (!namaKegiatan.value || !deskripsi.value || !image.value || !unitPendidikan.value) {
  ...
  try {
    ...
    await axios.post(`${API_BASE_URL}/api/v1/kegiatan`, formData, {
      headers: {
...
<template>
  <div class="flex">
    ...
    >
      <h2 class="text-2xl font-bold mb-4">Tambah Data Kegiatan</h2>
    ...
    <div class="mb-4">
      ...
      <input
        ...
        v-model="deskripsi"
        class="w-full p-2 border rounded-lg"
        placeholder="Masukkan Deskripsi (maks. 255 karakter)"
        required
      />
      <p :class="deskripsi.length > 255 ? 'text-red-500' : 'text-gray-500'">
        ...
        <div class="mt-6">
          ...
          <router-link to="/tbKegiatan" class="ml-4 text-gray-500 hover:underline">
            Batal
          </router-link>
        </div>
      </form>
