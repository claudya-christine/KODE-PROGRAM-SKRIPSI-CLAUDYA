...
const toggleDropdown = (linkName) => activeDropdown.value = activeDropdown.value === linkName ? null : linkName
...
<template>
  <header class="sticky top-0 z-50 bg-(--primary) py-4 px-5 md:px-0">
    <div class="container mx-auto flex justify-between items-center md:px-10">
      ...
      <nav 
        class="absolute md:relative top-full md:top-0 left-0 mt-0 md:mt-0 w-full md:w-auto 
        bg-(--primary) md:bg-transparent md:flex md:items-center 
        ...
        <ul class="text-white text-xs md:flex md:items-center py-3 md:py-0 px-1">
          ...
          >
            ...
            <div 
              ...
              class="block px-3 cursor-pointer py-1.5 pl-2 md:py-0 md:pl-0 hover:border-b-1 hover:border-yellow-500 flex items-center justify-between w-full"
              :class="{'font-bold text-yellow-200': link.children.some(child => route.path === child.link)}"
            >
              {{ link.name }}
            </div>
            ...
            <router-link 
              ...
              :class="{'font-bold text-yellow-200': route.path === link.link}"
            ...
            <ul 
              ...
              class="bg-[#294d87] md:bg-white md:absolute md:top-full md:left-0 md:mt-2 shadow-lg md:min-w-[200px] z-50 transition duration-300 ease-in-out"
            >
